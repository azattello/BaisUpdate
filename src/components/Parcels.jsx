...
const Parcels = () => {
    ...
    useEffect(() => {
        const fetchArchive = async () => {
            try {
                const response = await axios.get(`https://bais-cargo.kz:3001/api/archive/${userId}/getArchive`);
    ...
    const removeBookmark = async (trackNumber) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://bais-cargo.kz:3001/api/archive/${userId}/delete/${trackNumber}`, {
                    method: 'DELETE',
                });
